    ...
    public static void execute() {
        if (!hasClaimedReward()) {
            ...
            if (isInBasement()){
                ...
                if (stairs == null) {
                    Log.severe("Could not find museum stairs");
                    Main.walkTo(STAIRS_UP_TILE);
                } else {
                    if (stairs.interact("Walk-up")) {
                        Time.sleep(500, 1000);
                        if (Dialog.isOpen())
                            return;
                        Time.sleepUntil(() -> !isInBasement(), 5000);
                    }
                }
            }else{
